...
export default function JuryPanel() {
  ...
  const nextSlide = () => {
    setCurrentIndex(
      currentIndex === juryMembers.length - 3 ? 0 : currentIndex + 3
    );
  };

  const prevSlide = () => {
    setCurrentIndex(
      currentIndex === 0 ? juryMembers.length - 3 : currentIndex - 3
    );
  };
            ...
            >
              {juryMembers.map((jury, idx) => (
                <div
                  key={`${jury.name}-${idx}`}
                  className={`${
                    shouldAutoSlide ? "flex-shrink-0 w-1/3   px-4" : ""
                  }`}
                >
                  <div className="group bg-white rounded-lg shadow-md  transition-all duration-300 p-8 flex flex-col items-center text-center border border-slate-200">
                    {/* Image Container */}
