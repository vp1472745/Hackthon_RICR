...
const EditResultModal = ({ 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingResult?.hasResult) {
        // Update existing result
        await resultAPI.updateResult(editingResult._id, cleanData);
      ...
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
        ...
        <div className="mb-4">
          <p className="text-sm text-gray-600">
            Team: <span className="font-medium">{editingResult?.teamName || 'N/A'}</span>
          </p>
          <p className="text-sm text-gray-600">
            Code: <span className="font-medium">{editingResult?.teamCode || 'N/A'}</span>
          </p>
        </div>
