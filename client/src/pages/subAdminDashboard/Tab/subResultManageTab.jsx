    ...
    <div className="w-full max-w-4xl mx-auto py-4 sm:py-6 px-3 sm:px-4 lg:px-2 overflow-y-auto scrollbar-thin scrollbar-thumb-blue-300 scrollbar-track-blue-50" style={{maxHeight: 'calc(100vh - 48px)'}}>
      ...
      <div className="mt-4 sm:mt-6 grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4">
        <div className="bg-blue-50 rounded-lg p-3 text-center">
          <div className="text-lg sm:text-xl font-bold text-blue-700">{dummyResults.length}</div>
          <div className="text-xs text-blue-600">Total Teams</div>
        </div>
        <div className="bg-green-50 rounded-lg p-3 text-center">
          <div className="text-lg sm:text-xl font-bold text-green-700">{Math.max(...dummyResults.map(r => r.score))}</div>
          <div className="text-xs text-green-600">Highest Score</div>
        </div>
        <div className="bg-yellow-50 rounded-lg p-3 text-center">
          <div className="text-lg sm:text-xl font-bold text-yellow-700">{Math.min(...dummyResults.map(r => r.score))}</div>
          <div className="text-xs text-yellow-600">Lowest Score</div>
        </div>
        <div className="bg-purple-50 rounded-lg p-3 text-center">
          <div className="text-lg sm:text-xl font-bold text-purple-700">
            {Math.round(dummyResults.reduce((acc, curr) => acc + curr.score, 0) / dummyResults.length)}
          </div>
